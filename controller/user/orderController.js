...
const getOrderDetails = async (req, res) => {
  try {
    ...
    const userData = await user.findById(userId).select('name email phone profileImage');
    ...
    return res.render("user/orderDetailPage", {
      ...
      pageTitle: `Order #${orderDetails.orderId || orderDetails._id.toString().slice(-8).toUpperCase()}`
    });
...
const cancelOrder = async (req, res) => {
  try {
    ...
    if (!["Pending", "Processing"].includes(item.status))
      return res.status(400).json({ success: false, message: "Item cannot be cancelled now" });
...
export const downloadInvoice = async (req, res) => {
  try {
    ...
    doc
      ...
      .text(`Address   : ${address.flatNumber || ""} ${address.streetName || ""}`)
...
export default { getOrder, cancelOrder, getOrderDetails,downloadInvoice };